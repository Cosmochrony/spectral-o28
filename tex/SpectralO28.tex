...
\begin{document}
  ...
  \paragraph{Keywords.}
    ...
    exponent; finite-size adjustment; covariance operator; effective dimension; admissible trajectory; Born--Infeld
    saturation
  ...
  \subsection{What O25--O27 established}
    ...
    (ii) empirical fits carry no asymptotic meaning because the leading finite-size term
    $\eta\log q/\log n_1(q) \approx 1$ is itself $q$-dependent;
    (iii) the raw $\delta_{\mathrm{global}}(q)$ descends into the target window $[7.4, 10.6]$
    unaided, reaching $7.61$ at $q = 601$, while the finite-size adjustment
    $\eta\log q/\log n_1(q)$ of~\eqref{eq:dcorr} becomes progressively unnecessary and
    overcorrects beyond $q = 211$, sending the adjusted value below $7.4$.
    ...
    premises: the identification of the admissible image with the neutral traceless sector of the
    ambient observable space, which O27 assumes and derives nowhere, and the spinor carrier
    supplied conditionally by O23.
  ...
  \subsection{Extraction of \texorpdfstring{$n_1(q)$}{n1(q)} from checkpoints}
    ...
    \begin{table}[ht]
      ...
      \caption{BFS window depth and asymptotic ratio.
        ...
        $\eta \log q / \log n_1$: adjustment term~\eqref{eq:dcorr} with $\eta = 1/2$.
        $\delta_{\mathrm{corr}}$: adjusted value.
        Target window $[7.4, 10.6]$, taken by O25 from O10.}
      \label{tab:window}
  ...
  \subsection{Stability of \texorpdfstring{$\delta_{\mathrm{corr}}(q)$}{delta-corr(q)}}
    ...
    From Table~\ref{tab:window}, the adjustment term $\eta \log q / \log n_1(q)$ lies in
    $[0.96, 1.21]$ over the tested range.
    For all $q \in \{29, 61, 101, 151, 211\}$, the adjusted value
    $\delta_{\mathrm{corr}}(q)$ falls within $[7.4, 10.6]$, the target window that
    O25~\cite{Beau2026a29} takes from O10.
    Beyond that range the same adjustment overcorrects: O25 reports it sending the adjusted value
    to $6.53$ at $q = 601$, below the window that the raw observable reaches unaided.
    ...
    \begin{figure}[ht]
      ...
      \caption{Part~A: BFS window depth calibration over $q \in \{29,61,101,151,211\}$.
        ...
        and adjusted $\delta_{\mathrm{corr}}(q)$; over these five primes all adjusted values lie
        within the target window $[7.4,10.6]$ (shaded).}
      \label{fig:part_a}
    \end{figure}
  ...
  \subsection{What O28 establishes}
    ...
    First, the declared finite-size adjustment is stable over the calibration primes:
  ...
  O28 reports two measurements on the post-O27 programme.
